...
struct pad collect_data_about_next_pad(char* pad_str, char* buk, char* mrs, int current_pad_id, int prev_pad_it, char* error_desc)
{
	...
	char* singlePSPdata = ALLOC(sizeof(char) * PSP_DATA_SIZE + 1);
	char* singleReStr = ALLOC(sizeof(char) * REARRANGE_STR_SIZE + 1);
	...
	{
		/* Collect PSP points */
		memcpy(singlePSPdata, mrs + offset, PSP_DATA_SIZE);
		singlePSPdata[PSP_DATA_SIZE] = '\0';

		pspStartPoint = bindec(spae_substr(singlePSPdata, 0, PSP_START_POINT_SIZE));
		pspJumpPoint = bindec(spae_substr(singlePSPdata, 23, PSP_JUMP_POINT_SIZE));
		...
		memcpy(singleReStr, buk + offset, REARRANGE_STR_SIZE);
		singleReStr[REARRANGE_STR_SIZE] = '\0';
